...
export function re(strs, ...substs) {
    ...
    if (reStr.startsWith('/')) {
        ...
        if (lastSlashIndex === 0) {
            throw new SyntaxError('If the `re` string starts with a slash, it must end with a second slash and zero or more flags: '+reStr);
        }
 ...
 * @return {string}
 */
export const transformRaw = (str) => str.replace(/\\`/g, '`');
